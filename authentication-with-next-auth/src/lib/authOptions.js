    ...
    async signIn({ user, account, profile, email, credentials }) {
      try {
        if (account) {
          ...
          const userData = { providerAccountId, provider, user_email, image };
          console.log("FROM SIGNIN CALLBACK -----> ", userData);

          const userCollection = dbConnect(collectionNames.TEST_USER)
          const isUserExists = await userCollection.findOne({ providerAccountId })
